...
function Form({ onSearch }) {
  ...
  return (
    <div className="form-container">
      <p>Type in a valid BoardGameGeek user name to get game collection</p>
      <form onSubmit={submitHandler}>
        <input type="text" ref={formRef}></input>
        <button> Search </button>{" "}
        <label style={{ fontFamily: "Arial", fontSize: "0.8rem" }}>
          (For testing purpose, my user name is HaggbartSlark, but it will work
          with any valid id)
        </label>
      </form>
    </div>
      ...
      {error && (
        <center>
          <div style={{ fontSize: "4rem" }}>
            Oops, something went wrong. Did you type a valid user name? If you
            search too often, BGG will throttle your request. Wait a minute and
...
function Game({ el, background, handleActive, active }) {
  ...
  return (
    <div
      ...
      id={el.gameName}
      style={{
        backgroundColor: active ? "white" : background,
      }}
